    ...
    func loadFavList(isReset: Bool = true) async throws {
        ...
        if favList?.count ?? 0 > 0 {
            ...
            await MainActor.run {
                ...
                favdatas.forEach { favData in
                    if let dataUrl = URL(string: favData.cover) {
                        let model = TopShelfContentModel(id: String(favData.id), title: favData.title, description: favData.intro ?? "", imageURL: dataUrl)
